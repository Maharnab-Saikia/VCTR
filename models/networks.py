...
## Code taken and modified from https://github.com/lucidrains/ITTR-pytorch ##
...
class ChanLayerNorm(nn.Module):
    def __init__(self, embed_dim, eps=1e-5):
        super(ChanLayerNorm, self).__init__()

        self.eps = eps
        self.g = nn.Parameter(torch.ones(1, embed_dim, 1, 1))
        self.b = nn.Parameter(torch.zeros(1, embed_dim, 1, 1))
...
class HPB(nn.Module):
    ...
    def __init__(self, embed_dim, head_dim=32, num_heads=6, ffn_dim_multiplier=4, attn_height_top_k=8, attn_width_top_k=8, attn_dropout=0.0, ffn_dropout=0.0):
        ...
        self.attn_parallel_combine_out = nn.Conv2d(embed_dim * 2, embed_dim, 1)
...
class DPSA(nn.Module):
    ...
    def __init__(self, embed_dim, height_top_k=8, width_top_k=8, head_dim=32, num_heads=6, dropout=0.0):
        ...
        self.norm = ChanLayerNorm(embed_dim)
        self.to_qkv = nn.Conv2d(embed_dim, inner_dim * 3, 1, bias=False)

        self.height_top_k = height_top_k
        self.width_top_k = width_top_k

        self.dropout = nn.Dropout(dropout)
        self.to_out = nn.Conv2d(inner_dim, embed_dim, 1)
    ...
    def forward(self, x):
        ...
        if need_height_select_and_rank:
            ...
            top_h_indices = repeat(top_h_indices, 'b h -> b d h w', d=self.head_dim, w=k.shape[-1]) # (3, 20) -> (3, 64, 20, 47)
        ...
        if need_width_select_and_rank:
            ...
            top_w_indices = repeat(top_w_indices, 'b w -> b d h w', d=self.head_dim, h=k.shape[-2]) # (3, 47) -> (3, 64, 20, 47)
        ...
        out = rearrange(out, '(b h) (x y) d -> b (h d) x y', x=h, y=w, h=self.num_heads)
...
class VCTR(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64, num_blocks=9, opt=None):
        ...
        model += [
            HPB(
                embed_dim=opt.embed_dim,                    # dimension
                head_dim=opt.head_dim,                      # dimension per attention head
                num_heads=opt.num_heads,                    # number of attention heads
                attn_height_top_k=opt.attn_height_top_k,    # number of top indices to select along height, for the attention pruning
                attn_width_top_k=opt.attn_width_top_k,      # number of top indices to select along width, for the attention pruning
                ffn_dim_multiplier=opt.ffn_dim_multiplier,  # expansion factor of feedforward
                attn_dropout=opt.attn_dropout,              # attn dropout
                ffn_dropout=opt.ffn_dropout                 # feedforward dropout
